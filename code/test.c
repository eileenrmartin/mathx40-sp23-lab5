...
int main(int argc, char** argv){
	...
	int nTotal = 1001;
	int serialRank = 0; // this is the rank that'll have the serial vector initially created on it, to treat as root

	// all processes have a serial and parallel vector (but only serialRank has space allocated for the serial vector, rest are dummy variables)
	serialVector vecS;
	parallelVector vecP;

	// serial test (baseline which should match your parallel results, for testing purposes)	
	float sNorm;
	if(rank == serialRank){
		// full vector filled with random values in [0,1] and calculate its norm
	...
	int flag = initializeParallelVector(&vecP, nTotal); // allocate space for subset of data on each process
	scatterVector(&vecS, &vecP, serialRank); // fill in all vecP with data subsets
	float pNorm  = parallelNorm(&vecP, serialRank); // calculate norm in parallel and make sure all processes have a copy
