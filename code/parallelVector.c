...
// function scatters serial vector data on one process into a parallel distributed vector on all processes 
// (assume serialVector is initialized on root process and parallelVector is initialized on all processes already)
...
float parallelNorm(parallelVector *vecP, int root){
	...
	// send the local sums all into the root rank and add them all up, then turn into a float so you have the total norm on the root process
	double totalSum = 0;
	// ------------YOUR CODE HERE, 1 line--------------------------
	
	// ------------END YOUR CODE---------------------------	
	float totalNorm  = (float)totalSum;

	// send copy of the totalNorm from root to all other ranks
	// ------------YOUR CODE HERE, 1 line--------------------------
